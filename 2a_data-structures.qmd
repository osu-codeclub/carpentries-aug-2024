...
as not all information/measurements are always available for all samples.

Missing values are coded in R as `NA`
...
So, logicals can be used as if they were numbers, in which case `FALSE` represents 0
...
Then move on to the tasks below, which help you to fill in the blanks (`_____`).
